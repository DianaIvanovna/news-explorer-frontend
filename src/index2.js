...
(function () {
  ...
  const popupSuccessToggle = popupSuccess.querySelector('.popup__toggle-link'); // при нажатии Success закрывается Login открывается
  ...
  // eslint-disable-next-line
  const registration = new Popup(popupReg, popupRegButton, [popupRegClose,popupRegButton,popupRegToggle],[popupLoginToggle]);
  ...
  // eslint-disable-next-line
  // Данный попап нужно открывать в другом случае
  // const success = new Popup(popupSuccess,[popupSuccessClose,popupSuccessToggle]);
}());
